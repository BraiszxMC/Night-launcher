...
Launcher de Minecraft para el estudio de eventos. Version **beta**: al abrirlo
solo aparece el menu de inicio de sesion con Discord. Todavia no descarga ni
arranca Minecraft.

- Interfaz *liquid glass* en azul lapislazuli sobre grises oscuros.
- Fondo que deriva muy despacio y reacciona al puntero unos pocos pixeles.
- Ventana fija de 900x600: **no se puede agrandar ni encoger**, solo minimizar
  y cerrar. Barra de titulo propia, sin boton de maximizar y sin menu.
...
```bash
npm install
cp launcher.config.example.json launcher.config.json   # en Windows: copy
npm start
```
...
Tambien valen las variables de entorno `DISCORD_CLIENT_ID`,
`DISCORD_CALLBACK_PORT` y `DISCORD_GUILD_ID`, que tienen prioridad sobre el
JSON. `launcher.config.json` esta en el `.gitignore`: no subas tu configuracion.
...
Cada push lanza el workflow **Build**, que compila el instalador de Windows y el
AppImage de Linux. Se descargan desde la pestaña **Actions**: abre el run y mira
en *Artifacts* (caducan a los 30 dias).
...
El workflow **Release** compila y adjunta los instaladores a
https://github.com/BraiszxMC/Night-launcher/releases
...
Deja el instalador en `dist/`. Cada sistema compila lo suyo: el `.exe` hay que
generarlo en Windows.
...
src/
  ...
  renderer/
    ...
    renderer.js     logica del menu, puntero y estados
```
